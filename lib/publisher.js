...
export function resolvePublishContext(application = {}, publication = {}) {
  ...
  const url = URL.canParse(micropubEndpoint)
    ? micropubEndpoint
    : new URL(micropubEndpoint, `http://localhost:${port || "3000"}`).href;

  return { micropubEndpoint: url, me };
}
...
export async function publishPending(feed, itemsCollection, options) {
  ...
  if (feed.publish.since) {
    query.pubDate = { $gt: feed.publish.since };
  }
  ...
  console.log(
    `[RSS] Published ${published} item(s) from ${feed.title}${failed ? `, ${failed} failed` : ""}`,
  );
